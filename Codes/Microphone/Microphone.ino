...
void loop() {
  ...
  if (!recordingInProgress && (currentMillis - lastRecordingTime >= RECORD_TIME * 1000 + 60000)) { // 2 minutes after RECORD_TIME
    lastRecordingTime = currentMillis;
    recordAndUpload();
    recordingInProgress = true;
  }

  if (recordingInProgress && (currentMillis - lastRecordingTime >= RECORD_TIME * 1000 + 2000)) { // RECORD_TIME + extra time
...
void recordAndUpload() {
  ...
  // delay(RECORD_TIME * 1000 + 2000); // RECORD_TIME in seconds + extra time to ensure recording finishes


  // delay(120000); // Add a 10-second delay (adjust this value as needed)
...
void SPIFFSInit(){
  ...
  file.write(header, headerSize);
  Serial.println("WAV header written to file");
  listSPIFFS();
}
...
void i2s_adc_data_scale(uint8_t * d_buff, uint8_t* s_buff, uint32_t len)
{
    ...
    Serial.printf("Scaled %u bytes of data\n", len);
}
...
void i2s_adc(void *arg)
{
    ...
    while (flash_wr_size < FLASH_RECORD_SIZE) {
        ...
        // Add a delay to allow watchdog to reset
